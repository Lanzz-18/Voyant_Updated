...
// fetching rewards 
exports.getUserRewards = async (req, res) => { //api endpoint 
  try {
...
exports.unlockReward = async (req, res) => {
  ...
  // Checking if reward exists
    ...
    if (existingUnlock) { //if the condition returns true the if block will execute
    ...
    const userReward = new UserReward({
      ...
      rewardId,
      unlockedAt: new Date(),
      xpEarned: reward.xpPoints || 0,
      unlockProgress: 100,
    });
    
    await userReward.save(); // saving to database
    
    // TODO - make the exp udpate here  ( once the level scaling system is implemented as well)
    
    res.status(201).json({
      message: "Reward unlocked successfully",
      userReward: await UserReward.findById(userReward._id).populate('rewardId')
    });
  } catch (err) {
...
exports.getFavoriteRewards = async (req, res) => {
  try {
    const favorites = await UserReward.find({
      userId: req.params.userId,
      isFavorite: true
    }).populate('rewardId').sort({ unlockedAt: -1 });
