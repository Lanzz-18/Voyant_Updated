...
exports.processDialogueChoice = async (req, res) => {
  try {
    const { userId, mainQuestId, dialogueNodeId, optionId, userInput } = req.body;
    ...
    if (!currentSubQuestProgress.completedDialogueNodes.includes(dialogueNodeId)) {
    ...
    if (selectedOption.nextDialogueId) {
      ...
      currentSubQuestProgress.currentDialogueNodeId = selectedOption.nextDialogueId;
    }
    ...
    if (selectedOption.action === 'complete_quest') {
      ...
      currentSubQuestProgress.xpEarned = currentSubQuestProgress.subQuestId.rewards.xp || 0;
      
    //unlocking next sub-quest
      const currentIndex = userProgress.subQuestProgress.findIndex(
        subQuest => subQuest.subQuestId._id.toString() === currentSubQuestProgress.subQuestId._id.toString()
      );
